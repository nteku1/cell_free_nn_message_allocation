...
file1 = open('bLAST_cbaVER577_ENCAPSULATE_5_9_25_6_H_SIGMAPOINT1_NOISY_ESTIMATE_MMSEE_50000_ver62.txt', 'r')
#file1 = open('cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMAPOINT1_NOISY_ESTIMATE_MMSEE_50000_7_28_ExtendingTesting_ver62.txt', 'r')
...
file2 = open('bLAST_cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMAPOINT1_NOISY_ESTIMATE_MMSEE_part2_ver62.txt', 'r')
#file2 = open('cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMAPOINT1_NOISY_ESTIMATE_MMSEE_part2_7_28_ExtendingTesting_ver62.txt', 'r')
testlines = file2.readlines()

file3 = open('bLAST_cbaVER577_ENCAPSULATE_4_26_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_part2_ver62.txt', 'r')
...
for ep in range(total_episodes):
    ...
    while indexxx < len(testlines)-(num_user-1):
        ...
        sum_temp = np.zeros(shape=(1,num_APs))
        sum_den = np.zeros(shape=(1,num_APs))
        sum_temp_ACT = np.zeros(shape=(1,num_APs))
        sum_den_ACT = np.zeros(shape=(1,num_APs))
        sum_temp_u = np.zeros(shape=(1,num_APs))
        sum_den_u = np.zeros(shape=(1,num_APs))
        sum_temp_b = []
        sum_den_b = []
        for swag in range(num_user):
          ...
          ###########################den = 1+ ((p*ada_1)*(oneminussquared*chan_squared))
          den = ((p*ada_1)*(oneminussquared*chan_squared))
          den_ACT = ((p * ada_1) * (oneminussquared * chan_squared_ACT))
          ...
          sum_den_ACT[0] = sum_den_ACT[0] + den_ACT
          sum_temp_ACT[0] = sum_temp_ACT[0] + temp_ACT
        ...
        rate_ACT = ((B * np.log2((1 + ((sum_temp_ACT) / (1 + sum_den_ACT))))))


        un_dB_sinr = ((sum_temp)/(1+sum_den))
        un_dB_sinr_ACT = ((sum_temp_ACT) / (1 + sum_den_ACT))
        '''
        if indexxx > 0:
          print('Original')
          print(rate)
        ...
        for sew in range(num_user):
          ...
          new_alphasss_ACT = actions[sew][actions[sew] != 0]
          ...
          if tezerk != tezerk2:
            print('ACT Size Damn it')
            print('chan squared')
            print(chan_squared_ACT)
            print('Alphas')
            print(alphasss)
            print('Temp')
            print(temp_ACT)
      ...
      #state, reward, done, info = env.step(action)
          ...
          trans_latencies_co.append(np.max(beta2 * (product / new_rate_ACT)))
       ...
       '''
        ...
        #### BEST Method
        for great in range(num_user):
          #best_id = np.argmax(np.abs(complex_vec_total_p2_ACT)[great])
...
plt.ylabel("CDF")
plt.legend(['Proposed Method - Using CSI with added noise ('r'$\sigma=1)$', 'Uniform Allocation', 'Greedy Allocation'])#,'Interior Point'])
plt.show()
