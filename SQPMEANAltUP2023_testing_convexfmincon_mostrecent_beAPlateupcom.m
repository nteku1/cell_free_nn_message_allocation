...
nbrOfSetups = 4; %%% 20 active set was3;   % 4;%3 30;
aaaaaa = 0;
...
L = 25;%6;%25;%50;%10; %25;%20;%20; %10; %20;%25;%20; %25; %20;% 10; %original: 10
...
%fileidfun = fopen('cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMA1_NOISY_ESTIMATE_MMSEE_part2_7_28_ExtendingTesting_ver62.txt','r');
...
for iiii = 1:6000%50000%6000%4000%300000%1000
...
% % % % % % % % % % %        tde = max(max(beta2*((alphasss_0*S_u)./(B*log2(1+(sum(p*ada_1*alphasss_0.^2.*all_states.^2,1)./(1+sum(p*ada_1*((1-alphasss_0).^2).*all_states.^2,1)))))),[],1));
        ...
        %lb = zeros(1,L*K);%0;
        lb = zeros(1,L*K) + 0.00000001 * ones(1,L*K);%0;
        ub = ones(1,L*K);%;1;
...
%active-set works but takes too long
...
%       options = optimoptions(@fmincon,'Algorithm','active-set','MaxFunEvals',8000,'StepTolerance',1e-8,'TolCon',1e-8);%'MaxIter',2000,'TolCon',1e-10); %2000 TolCon 1e-10 TolX

%  options = optimoptions(@fmincon,'Algorithm','sqp','MaxFunEvals',6000,'StepTolerance',1e-10,'TolCon',1e-15);
% options = optimoptions(@fmincon,'Algorithm','sqp','MaxFunEvals',4500,'StepTolerance',1e-10,'TolCon',1e-20);
% %       options = optimoptions(@fmincon,'Algorithm','active-set','MaxFunEvals',4500,'StepTolerance',1e-10,'TolCon',1e-4); 
