...
L = 25;%50;%6;%50;%6;%25;%50;%25;%25;%20;%10;%25;%20; %25; %20;% 10; %original: 10
...
% % % % % % % % fileidfun = fopen('VER19_2023_10AP_20_UE_PL3_100m_50000SHAD_part2.txt','r');
%%%%%%%%%%%%%%%fileidfun = fopen('bbbbLASTT_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_STOP_bbPLS_vREDO_bbvMOO_PLS_STOP_VER819_NOMOvACTUAL_LARGE_BPL3_1000m_VER15_25AP_6UE_SHAD_Hmat_scenario3_10APs_Multi_2_users_FUNFINALACTUREG_complex_part2_7_28_ExtendingTesting_ver62.txt','r');
% fileidfun = fopen('TESTINGG_4_25_TOO_LARGE_WHY_VER5_FINAL_TODAY_50_6_part2.txt','r');
%fileidfun = fopen('cbaVER577_ENCAPSULATE_4_26_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_part2_7_28_ExtendingTesting_ver62.txt','r');
% fileidfun = fopen('VER577_NOW_VER51_2023_6AP_50_UE_PL3_100m_50000SHAD_part2.txt','r');
% fileidfun = fopen('TESTINGG_4_30_TOO_LARGE_WHY_VER5_FINAL_TODAY_50_6_part2.txt','r');
% fileidfun = fopen('LAST_a4_30_VER577_NOW_VER51_2023_6AP_50_UE_PL3_100m_50000SHAD_part2.txt','r');
fileidfun = fopen('bLAST_cbaVER577_ENCAPSULATE_4_26_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_part2_ver62.txt','r');
...
for iiii = 1:6000%50000%6000%50000%6000%20000%6000%4000%300000%1000
...
% % % % % % % % % % % % % % % % % % %              fun = @(alphasss) max((beta1*(sum(((alphasss.*S_u*C)/f),2))).' + (beta2*(sum((alphasss*S_u),2).'./sum((B*log2(1+(p* abs(sum(ada_1*alphasss.*all_states,2).').^2./(1+p* mean(abs(sum(ada_1*(1-alphasss).*all_states,2).').^2))))),1))));  
             ...
             fun = @(alphasss) max((beta1*(sum(((alphasss.*S_u*C)/f),2)))) + mean(max(beta2*((alphasss*S_u)./(B*log2(1+(sum(p*ada_1*alphasss.^2.*all_states.^2,2)./(1+sum(p*ada_1*((1-alphasss).^2).*all_states.^2,2)))))),[],1));  
...
% % % % % % % % % % %        tde = max(max(beta2*((alphasss_0*S_u)./(B*log2(1+(sum(p*ada_1*alphasss_0.^2.*all_states.^2,1)./(1+sum(p*ada_1*((1-alphasss_0).^2).*all_states.^2,1)))))),[],1));
        ...
        lb = zeros(1,L*K);%0;
        ub = ones(1,L*K);%;1;
%  50       Aeq = [[ones(1,L) zeros(1,K*L-L)];[zeros(1,L) ones(1,L) zeros(1,K*L-2*L)]; [zeros(1,2*L) ones(1,L) zeros(1,K*L-3*L)]; [zeros(1,3*L) ones(1,L) zeros(1,K*L-4*L)];  [zeros(1,4*L) ones(1,L) zeros(1,K*L-5*L)]; [zeros(1,5*L) ones(1,L) zeros(1,K*L-6*L)]; [zeros(1,6*L) ones(1,L) zeros(1,K*L-7*L)];...
%               [zeros(1,7*L) ones(1,L) zeros(1,K*L-8*L)]; [zeros(1,8*L) ones(1,L) zeros(1,K*L-9*L)]; [zeros(1,9*L) ones(1,L) zeros(1,K*L-10*L)]; [zeros(1,10*L) ones(1,L) zeros(1,K*L-11*L)]; [zeros(1,11*L) ones(1,L) zeros(1,K*L-12*L)]; [zeros(1,12*L) ones(1,L) zeros(1,K*L-13*L)]; [zeros(1,13*L) ones(1,L) zeros(1,K*L-14*L)];...
%                [zeros(1,14*L) ones(1,L) zeros(1,K*L-15*L)]; [zeros(1,15*L) ones(1,L) zeros(1,K*L-16*L)]; [zeros(1,16*L) ones(1,L) zeros(1,K*L-17*L)]; [zeros(1,17*L) ones(1,L) zeros(1,K*L-18*L)]; [zeros(1,18*L) ones(1,L) zeros(1,K*L-19*L)];[zeros(1,19*L) ones(1,L) zeros(1,K*L-20*L)]; [zeros(1,20*L) ones(1,L) zeros(1,K*L-21*L)]
%               [zeros(1,21*L) ones(1,L) zeros(1,K*L-22*L)]; [zeros(1,22*L) ones(1,L) zeros(1,K*L-23*L)]; [zeros(1,23*L) ones(1,L) zeros(1,K*L-24*L)]; [zeros(1,24*L) ones(1,L) zeros(1,K*L-25*L)]; [zeros(1,25*L) ones(1,L) zeros(1,K*L-26*L)];[zeros(1,26*L) ones(1,L) zeros(1,K*L-27*L)]; [zeros(1,27*L) ones(1,L) zeros(1,K*L-28*L)]
%               [zeros(1,28*L) ones(1,L) zeros(1,K*L-29*L)]; [zeros(1,29*L) ones(1,L) zeros(1,K*L-30*L)]; [zeros(1,30*L) ones(1,L) zeros(1,K*L-31*L)]; [zeros(1,31*L) ones(1,L) zeros(1,K*L-32*L)]; [zeros(1,32*L) ones(1,L) zeros(1,K*L-33*L)];[zeros(1,33*L) ones(1,L) zeros(1,K*L-34*L)]; [zeros(1,34*L) ones(1,L) zeros(1,K*L-35*L)]
%               [zeros(1,35*L) ones(1,L) zeros(1,K*L-36*L)]; [zeros(1,36*L) ones(1,L) zeros(1,K*L-37*L)]; [zeros(1,37*L) ones(1,L) zeros(1,K*L-38*L)]; [zeros(1,38*L) ones(1,L) zeros(1,K*L-39*L)]; [zeros(1,39*L) ones(1,L) zeros(1,K*L-40*L)];[zeros(1,40*L) ones(1,L) zeros(1,K*L-41*L)]; [zeros(1,41*L) ones(1,L) zeros(1,K*L-42*L)]
%               [zeros(1,42*L) ones(1,L) zeros(1,K*L-43*L)]; [zeros(1,43*L) ones(1,L) zeros(1,K*L-44*L)]; [zeros(1,44*L) ones(1,L) zeros(1,K*L-45*L)]; [zeros(1,45*L) ones(1,L) zeros(1,K*L-46*L)]; [zeros(1,46*L) ones(1,L) zeros(1,K*L-47*L)];[zeros(1,47*L) ones(1,L) zeros(1,K*L-48*L)]; [zeros(1,48*L) ones(1,L) zeros(1,K*L-49*L)]
%               [zeros(1,K*L-L) ones(1,L)]];        
%         
        
        
%  20        Aeq = [[ones(1,L) zeros(1,K*L-L)];[zeros(1,L) ones(1,L) zeros(1,K*L-2*L)]; [zeros(1,2*L) ones(1,L) zeros(1,K*L-3*L)]; [zeros(1,3*L) ones(1,L) zeros(1,K*L-4*L)];  [zeros(1,4*L) ones(1,L) zeros(1,K*L-5*L)]; [zeros(1,5*L) ones(1,L) zeros(1,K*L-6*L)]; [zeros(1,6*L) ones(1,L) zeros(1,K*L-7*L)];...
%               [zeros(1,7*L) ones(1,L) zeros(1,K*L-8*L)]; [zeros(1,8*L) ones(1,L) zeros(1,K*L-9*L)]; [zeros(1,9*L) ones(1,L) zeros(1,K*L-10*L)]; [zeros(1,10*L) ones(1,L) zeros(1,K*L-11*L)]; [zeros(1,11*L) ones(1,L) zeros(1,K*L-12*L)]; [zeros(1,12*L) ones(1,L) zeros(1,K*L-13*L)]; [zeros(1,13*L) ones(1,L) zeros(1,K*L-14*L)];...
%                [zeros(1,14*L) ones(1,L) zeros(1,K*L-15*L)]; [zeros(1,15*L) ones(1,L) zeros(1,K*L-16*L)]; [zeros(1,16*L) ones(1,L) zeros(1,K*L-17*L)]; [zeros(1,17*L) ones(1,L) zeros(1,K*L-18*L)]; [zeros(1,18*L) ones(1,L) zeros(1,K*L-19*L)];[zeros(1,K*L-L) ones(1,L)]];
...
% % %        %'MaxFunEvals',4500,'TolCon', 1e-10'  6500
     ...
     const_outputs(iiii/K) = max((beta1*(sum(((const_alphasss.*S_u*C)/f),2)))) + mean(max(beta2*((const_alphasss*S_u)./(B*log2(1+(sum(p*ada_1*const_alphasss.^2.*all_states.^2,2)./(1+sum(p*ada_1*((1-const_alphasss).^2).*all_states.^2,2)))))),[],1));
...
% % % % % % % % % % %      const_outputs(iiii) = max((beta1*(sum(((const_alphasss.*D*C)/f),2))).' + (beta2*(sum((const_alphasss*S_u),2).'./sum((B*log2(1+(p* abs(sum(ada_1*const_alphasss.*H_AP,2).').^2./(1+p* mean(abs(sum(ada_1*(1-const_alphasss).*H_AP,2).').^2))))),1))));
     ...
     best_outputs(iiii/K) = max((beta1*(sum(((best_alphasss.*S_u*C)/f),2)))) + mean(max(beta2*((best_alphasss*S_u)./(B*log2(1+(sum(p*ada_1*best_alphasss.^2.*all_states.^2,2)./(1+sum(p*ada_1*((1-best_alphasss).^2).*all_states.^2,2)))))),[],1));
