...
%fileidfun = fopen('bbbbLASTT_25_6_H_SIGMA5_NOISY_ESTIMATE_MMSEE_STOP_bbPLS_vREDO_bbvMOO_PLS_STOP_VER819_NOMOvACTUAL_LARGE_BPL3_1000m_VER15_25AP_6UE_SHAD_Hmat_scenario3_10APs_Multi_2_users_FUNFINALACTUREG_complex_part2_7_28_ExtendingTesting_ver62.txt','r');
% fileidfun = fopen('cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMAPOINT1_NOISY_ESTIMATE_MMSEE_part2_7_28_ExtendingTesting_ver62.txt','r');
% fileidfun_ACTUAL = fopen('cbaVER577_ENCAPSULATE_4_26_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_part2_7_28_ExtendingTesting_ver62.txt','r');

fileidfun = fopen('bLAST_cbaVER577_ENCAPSULATE_4_26_25_6_H_SIGMA1_NOISY_ESTIMATE_MMSEE_part2_ver62.txt','r');
fileidfun_ACTUAL = fopen('bLAST_cbaVER577_ENCAPSULATE_4_26_25_6_H_ACTUAL_NOT_ESTIMATE_MMSEE_part2_ver62.txt','r');
...
%          fun = @(alphasss) max((beta1*((alphasss.*D*C)/f)) + (beta3*((alphasss.*(L-1)*D)/sum((B*log2(1+((p*ada_1*alphasss.^2.*abs(Hhat_AP).^2.')./(1+p*ada_1*((1-alphasss).^2).*abs(Hhat_AP).^2.')))))))  + (beta4*(((L-1)*(length(Hhat_AP)+length(alphasss)))/sum((B*log2(1+((p*ada_1*alphasss.^2.*abs(Hhat_AP).^2.')./(1+p*ada_1*((1-alphasss).^2).*abs(Hhat_AP).^2.'))))))) + (beta2*((alphasss*S_u)./(B*log2(1+((p*ada_1*alphasss.^2.*abs(Hhat_AP).^2.')./(1+p*ada_1*((1-alphasss).^2).*abs(Hhat_AP).^2.')))))));
              ...
              fun_ACT = @(alphasss) max((beta1*(sum(((alphasss.*S_u*C)/f),2)))) + mean(max(beta2*((alphasss*S_u)./(B*log2(1+(sum(p*ada_1*alphasss.^2.*all_states_ACTUAL.^2,2)./(1+sum(p*ada_1*((1-alphasss).^2).*all_states_ACTUAL.^2,2)))))),[],1));  
...
% 20 ues          Aeq = [[ones(1,L) zeros(1,K*L-L)];[zeros(1,L) ones(1,L) zeros(1,K*L-2*L)]; [zeros(1,2*L) ones(1,L) zeros(1,K*L-3*L)]; [zeros(1,3*L) ones(1,L) zeros(1,K*L-4*L)];  [zeros(1,4*L) ones(1,L) zeros(1,K*L-5*L)]; [zeros(1,5*L) ones(1,L) zeros(1,K*L-6*L)]; [zeros(1,6*L) ones(1,L) zeros(1,K*L-7*L)];...
...
% % %        %'MaxFunEvals',4500,'TolCon', 1e-10'  6500
      ...
      options = optimoptions(@fmincon,'Algorithm','sqp','MaxFunEvals',4500,'StepTolerance',1e-10,'TolCon',1e-10);%'MaxIter',2000,'TolCon',1e-10); %2000 TolCon 1e-10 TolX
...
%      outputs(iiii/K) =  fun(alphasss);
    outputs(iiii/K) =  fun_ACT(alphasss);
